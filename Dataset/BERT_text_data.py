...
if will_dump_objects:
    ...
    with open("./data_%s.labels" % cfg_ds, "wb") as f:
        pkl.dump([label2idx, idx2label], f)
    with open("./data_%s.y" % cfg_ds, "wb") as f:
        pkl.dump(y, f)
    with open("./data_%s.y_prob" % cfg_ds, "wb") as f:
        pkl.dump(y_prob, f)
    with open("./data_%s.train_y" % cfg_ds, "wb") as f:
        pkl.dump(train_y, f)
    with open("./data_%s.train_y_prob" % cfg_ds, "wb") as f:
        pkl.dump(train_y_prob, f)
    with open("./data_%s.valid_y" % cfg_ds, "wb") as f:
        pkl.dump(valid_y, f)
    with open("./data_%s.valid_y_prob" % cfg_ds, "wb") as f:
        pkl.dump(valid_y_prob, f)
    with open("./data_%s.test_y" % cfg_ds, "wb") as f:
        pkl.dump(test_y, f)
    with open("./data_%s.test_y_prob" % cfg_ds, "wb") as f:
        pkl.dump(test_y_prob, f)
    with open("./data_%s.tfidf_list" % cfg_ds, "wb") as f:
